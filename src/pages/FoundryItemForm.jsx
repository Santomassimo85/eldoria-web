            ...
            <div className="dmt-li" key={p.id}>
              ...
              <div className="d">
                ...
                {p.origin === "crafting" && p.craft && (
                  <span className="fdy-craft-note"> · {PROFESSIONI.find(x => x.key === p.craft.profession)?.name || p.craft.profession} · pregiatura {p.craft.tier} (mirata {p.craft.targetTier}) · d20 {p.craft.d20} → {p.craft.total} · d12 {p.craft.d12}{p.craft.cost ? ` · materiali ${p.craft.cost}` : ""}{p.craft.work ? ` · ⏱ lavoro ${p.craft.work}` : ""}{p.craft.componentBonus ? ` · componenti +${p.craft.componentBonus} al tiro` : ""}{p.craft.componentProofs?.length ? ` · possesso: ${p.craft.componentProofs.join("; ")}` : ""}{p.craft.toolsProof ? ` · strumenti ${p.craft.toolsProof}` : ""}{p.craft.componentEffects?.length ? ` · ⚠ DA DECIDERE: ${p.craft.componentEffects.join("; ")}` : ""}{p.craft.enhancer ? ` · potenziatore ${ENHANCERS.find(e => e.key === p.craft.enhancer)?.name || p.craft.enhancer}${p.craft.enhancerProof ? ` (${p.craft.enhancerProof})` : p.craft.enhancerOwned ? " (dichiarato, non verificato)" : ""}` : ""}{p.craft.mods?.length ? ` · condizioni: ${p.craft.mods.join(", ")}` : ""}{p.craft.note ? ` · nota: "${p.craft.note}"` : ""}</span>
                )}
