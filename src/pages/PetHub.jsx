...
function PetCompagniPanel({ uid, isMaster }) {
  ...
  const [message, setMessage] = useState(null);
  const [enlargedPet, setEnlargedPet] = useState(null); // pet object
  ...
  return (
    <div className="ph-care">
      ...
      {/* Enlarged pet card — TCG-style detail overlay */}
      {enlargedPet && (
        <PetCardDetail
          pet={enlargedPet}
          onClose={() => setEnlargedPet(null)}
        />
      )}
    </div>
  );
...
/* ============================================================
   ENLARGED PET CARD — TCG-style detail overlay
   ============================================================ */
function PetCardDetail({ pet, onClose }) {
  const sp = PET_SPECIES[pet.speciesKey];
  if (!sp) return null;
  const lvl   = levelFromExp(pet.exp || 0);
  const stats = petStatsAtLevel(pet.speciesKey, lvl, pet.bonusStats);
  ...
  return (
    <div className="ph-detail-overlay" onClick={onClose}>
      ...
      >
        ...
        {/* Title row: name + cost-like level pip */}
        ...
        <div className="ph-detail-species">
          <em>{sp.name}</em>
        </div>
        ...
        <div className="ph-detail-stats">
          ...
          <StatCell icon="💀" label="Sconfitte" value={pet.losses || 0} />
          {ageDays != null && (
            <StatCell icon="🥚" label="Età" value={ageDays === 0 ? "oggi" : `${ageDays}g`} />
          )}
        </div>
